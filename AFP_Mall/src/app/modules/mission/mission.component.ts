...
export class MissionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.oauthService.cookiesGet('userName').s !== null) {
      ...
      switch (mission.Mission_ClickState) {
        case 2: // 前往任務(進階任務)
          ...
          if (mission.Mission_CurrentURL.indexOf('/feedback/?') > 0) {
            // const strUser = '?customerInfo=' + sessionStorage.getItem('CustomerInfo') + '&userCode=' + this.oauthService.cookiesGet('userCode').s + '&userName=' + this.oauthService.cookiesGet('userName').s + '&loginType=1';
            // const device = { system: '', isApp: this.appService.isApp !== null ? strUser + '&isApp=1' : '' };
            const strUser = '?M_idToken=' + this.oauthService.cookiesGet('idToken').s + '&userCode=' + this.oauthService.cookiesGet('userCode').s + '&userName=' + this.oauthService.cookiesGet('userName').s + '&loginType=1';
            const device = {
              system: '',
